...
DT::datatable(dmrcrc %>% arrange(Date), width = '100%',  
              extensions = 'Buttons', 
              options = list(scrollX = TRUE, dom = 'Bfrtip', buttons = c('csv')))
```

```{r, fig.cap='Rings dataset'}
DT::datatable(dr %>% arrange(Date), width = '100%',  
              extensions = 'Buttons', 
              options = list(scrollX = TRUE, dom = 'Bfrtip', buttons = c('csv')))
```
...
subplot(ggplotly(plot1, width=wid, height = hei*3/2), 
        ...
        ggplotly(plot6, width=wid, height = hei*3/2), nrows=3)
```
...
p <- data.frame(x=(1:10),y=nextSeasonI) %>% ggplot( aes(y = y, x=x)) +
  ...
  theme(aspect.ratio=9/16, legend.position="none")  + labs(y='Probability of recapture', color = "Adult", x='Year after capture')
